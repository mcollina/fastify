      ...
      avvio.once('preReady', () => {
        ...
        if (opts.hooks) {
          for (const hook in opts.hooks) {
            hooks[hook] = []
              .concat(opts.hooks[hook] || [])
              .map(h => {
                if (lifecycleHooks.indexOf(hook) === -1) {
                  throw new Error(`${hook} hook not supported as a route-level hook!`)
                }
                this[kHooks].validate(hook, h)
                this[kHooks].validateAsyncHookParams(hook, h)
                return h
              })
          }
        } else {
